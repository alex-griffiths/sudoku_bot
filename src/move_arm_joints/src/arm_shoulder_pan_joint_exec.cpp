                   ...
                   move_arm_joints::move_and_confirm::Response &resp)
{
	...
	req_string << "./arm_shoulder_pan_servo " << req.move;

	cout << "arm_shoulder_pan_servo " << req.move << endl;

	system(req_string.str().c_str());
	
	// Send response
	ifstream log_file;
	log_file.open("arm_shoulder_pan_servo_confirm.log");
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle nh;
	ros::ServiceServer myServer = nh.advertiseService("arm_shoulder_pan_joint_service", &move_and_confirm);
